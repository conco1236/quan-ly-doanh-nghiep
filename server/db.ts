...
export function summarizeMaintenance(schedules: Array<{ nextDueAt: Date; status: string }>, tickets: Array<{ status: string; priority: string; cost: string | number }>, now = new Date()) { return { overdue: schedules.filter(row => row.status === "active" && new Date(row.nextDueAt) < now).length, openTickets: tickets.filter(row => row.status === "open" || row.status === "in_progress").length, criticalTickets: tickets.filter(row => row.priority === "critical" && row.status !== "resolved" && row.status !== "cancelled").length, totalCost: tickets.filter(row => row.status !== "cancelled").reduce((sum, row) => sum + (Number(row.cost) || 0), 0) }; }
...
export function calculateRecipeCost(recipeRows: Array<{ beerTypeId: number; ingredientId: number; quantity: string | number }>, unitCosts: Map<number, number>) { return recipeRows.reduce((sum, row) => sum + (Number(row.quantity) || 0) * (unitCosts.get(row.ingredientId) ?? 0), 0); }
export async function getCostingReport(ownerId?: number) { const db = await getDb(); if (!db) return { recipes: [], batches: [] }; const [recipeRows, ingredientRows, purchaseRows, batchRows] = await Promise.all([db.select().from(recipes).where(ownerId ? eq(recipes.createdBy, ownerId) : undefined), db.select().from(ingredients), db.select().from(purchaseOrderItems), db.select().from(productionBatches).where(ownerId ? eq(productionBatches.createdBy, ownerId) : undefined)]); const unitCosts = new Map<number, number>(); for (const ingredient of ingredientRows) { const purchases = purchaseRows.filter(row => row.ingredientId === ingredient.id); const latest = purchases[purchases.length - 1]; unitCosts.set(ingredient.id, latest ? Number(latest.unitPrice) : 0); } const recipeCosts = Array.from(new Set(recipeRows.map(row => row.beerTypeId))).map(beerTypeId => ({ beerTypeId, cost: calculateRecipeCost(recipeRows.filter(row => row.beerTypeId === beerTypeId), unitCosts) })); const batches = batchRows.map(batch => { const unitCost = recipeCosts.find(row => row.beerTypeId === batch.beerTypeId)?.cost ?? 0; const actualQuantity = Number(batch.actualQuantity) || 0; return { id: batch.id, batchCode: batch.batchCode, beerTypeId: batch.beerTypeId, status: batch.status, plannedQuantity: Number(batch.plannedQuantity) || 0, actualQuantity, estimatedCost: unitCost * (actualQuantity || Number(batch.plannedQuantity) || 0) }; }); return { recipes: recipeCosts, batches }; }
